...
from chat.agent.deep_research.promptmodule import (
    SubtasksDecomposition,
    WebExtraction,
    FollowupJudge,
    ReflectFailure,
)
from chat.agent.deep_research.utils import (
...
class DeepResearchAgent(ReActAgent):
    ...
    def __init__(
        ...
        memory: MemoryBase,
        search_mcp_client: StatefulClientBase | None = None,
        ...
        self.search_function = getattr(search_mcp_client, "search_tool_name", "search") if search_mcp_client else "search"
        self.extract_function = getattr(search_mcp_client, "extract_tool_name", "extract") if search_mcp_client else "extract"
    ...
    async def _ensure_mcp_initialized(self) -> None:
        ...
        if not self._mcp_initialized:
            if self._search_mcp_client is not None:
