...
def truncate_search_result(
    ...
    """Truncate tool results to fit context window limits."""
    for i, val in enumerate(res):
        if isinstance(val, dict) and "text" in val:
            ...
            res[i] = truncate_by_words(val, max_tool_results_words)
...
def load_prompt_dict() -> dict:
    ...
    prompt_dict["add_note"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_worker_additional_sys_prompt.md",
        ),
        return_json=False,
    )
    ...
    prompt_dict["tool_use_rule"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_tool_usage_rules.md",
        ),
        return_json=False,
    )
    ...
    prompt_dict["decompose_sys_prompt"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_decompose_subtask.md",
        ),
        return_json=False,
    )
    ...
    prompt_dict["expansion_sys_prompt"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_deeper_expansion.md",
        ),
        return_json=False,
    )
    ...
    prompt_dict["summarize_sys_prompt"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_inprocess_report.md",
        ),
        return_json=False,
    )
    ...
    prompt_dict["reporting_sys_prompt"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_deepresearch_summary_report.md",
        ),
        return_json=False,
    )
    ...
    prompt_dict["reflect_sys_prompt"] = get_prompt_from_file(
        file_path=os.path.join(
            cur_dir,
            "prompts/prompt_reflect_failure.md",
        ),
        return_json=False,
    )
