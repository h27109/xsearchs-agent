...
两后端各自独立读取此变量（chat/ 和 manage/ 各有自己的 `get_data_dir()`）。不设则回退到项目根下的 `data/`。

## Startup

```bash
cd /root/xsearchs-agent
...
2. `ensure_session()` 自动创建 session 行（如不存在）
3. `AsyncSQLAlchemyMemory` 从 SQLite 加载历史
4. LLM 推理(DeepSeek/MiniMax)，需要时调用 MCP 工具
5. SSE 流式返回，消息自动持久化到 message 表
