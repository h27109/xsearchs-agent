...
function App() {
  ...
  if (page.stage === 'login') return <LoginPage onLogin={(auth) => setPage({ stage: 'chat', auth })} onNeedSetPassword={(userId) => setPage({ stage: 'set_password', userId })} />;
  if (page.stage === 'set_password') return <SetPasswordPage userId={page.userId} onSuccess={(auth) => setPage({ stage: 'chat', auth })} />;
  return <Chat auth={page.auth} onLogout={() => { clearAuth(); setPage({ stage: 'login' }); }} />;
}
